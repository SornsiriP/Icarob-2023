...
class XarmIK(gym.Env):
    ...
    def reset(self):
        ...
        self.object_id = p.loadSoftBody("ball.obj", simFileName = "ball.vtk", basePosition = [2.5,-0.1,0.2], scale =0.25, mass = 1, useNeoHookean = 1, NeoHookeanMu = 800, NeoHookeanLambda = 1200, NeoHookeanDamping = 0.001, useSelfCollision = 1, frictionCoeff = .9, collisionMargin = 0.001)
        # self.object_id = p.loadSoftBody("tube.vtk", simFileName = "tube.vtk", basePosition = [1.5,-0.1,0.13],baseOrientation = [0,0,1,1], scale =0.4, mass = 1, 
        ...
        # self.object_id = self.add_noodle(pos = [2,-3,1], orientation=[0.5,0,0,1])
        # self.add_noodle(pos = [2,-1,1], orientation=[0.8,0,0,1])
        # self.add_noodle(pos = [1.5,-3,1], orientation=[.3,1,0,1])
        # self.add_noodle(pos = [3,-3,1], orientation=[0.5,.6,0.6,1])

        print(f"{p.getLinkState(self.xarm_id,0)}")
        p.changeDynamics(self.object_id, -1,mass=0.1, lateralFriction=10)
        p.changeDynamics(self.xarm_id, 11, lateralFriction=0.5,spinningFriction=.5, rollingFriction=.5)
        p.changeDynamics(self.xarm_id, 14, lateralFriction=0.5,spinningFriction=.5, rollingFriction=.5)
        p.configureDebugVisualizer(p.COV_ENABLE_RENDERING,1)
        self.observation = [-1]*7
        return self.observation

    def add_noodle(self, pos, orientation):
        id = p.loadSoftBody("Rope_new.obj",
            # simFileName="Rope_vtk_new.vtk",
            basePosition=pos,
